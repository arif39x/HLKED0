//
// Blowfish - Blowfish encryption for mIRC
...
char *encrypt_string(const char *key, const char *str)
{
	DWORD left, right;
	unsigned char *p;
	char *s, *dest, *d;
...
// Returned string must be freed when done with it!
static char *decrypt_string(const char *key, const char *str)
{
	DWORD left, right;
	char *p, *s, *dest, *d;
	int i;
...
void bfEncrypt (const char *key, const char *str, char *out)
{
	memset(out, 0, sizeof(out));
	strcpy(out, encrypt_string(key,str));
}

void bfDecrypt (const char *key, const char *str, char *out)
{
	memset(out, 0, sizeof(out));
	strcpy(out, decrypt_string(key,str));
}