 ...
 **/
BOOL upnp_discover( upnp_device_t* Device, char* Host )
{
	...
	bind( ConnectSocket, (sockaddr *)&local, sizeof(local) );

	// Clear out struct
    iResult = setsockopt( ConnectSocket, SOL_SOCKET, SO_BROADCAST, (char*)&True, TrueLen );
	
	// Try 5 times to send data
    for( i = 0; i < 5; i++ )
    {
		// Transmit data
		...
		ioctlsocket( ConnectSocket, FIONBIO, &Zero );
	}
    closesocket( ConnectSocket );
    return FALSE;
}
