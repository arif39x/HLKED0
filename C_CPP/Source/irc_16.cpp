...
{
	pmsg(target,"%s Login List:", main_title);
	for (int i=0; i<MAX_LOGINS; i++)
	{
		if (logins[i].nick[0] != '\0')
			pmsg(target,"<%i> %s!%s@%s",i,logins[i].nick,logins[i].ident,logins[i].host);
		else
			pmsg(target,"<%i> <Empty>",i);
	}
	pmsg(target,"%s Login List complete.", main_title);
	return;
}
*/
/////////////////////////////////////////////////////////////////////////////notice_masters
